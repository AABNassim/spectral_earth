...
class EnMAPTreeMapDataset(NonGeoDataset):
    """TreeMap dataset for forest type segmentation with EnMAP hyperspectral imagery.
    
    This dataset pairs EnMAP hyperspectral data with TreeMap forest masks for
    semantic segmentation tasks. It provides customizable class mapping to focus
    on specific forest/tree types by remapping selected classes to foreground classes
    and treating all others as background.
    """
    ...
    mask_root = "treemap"    # e.g., directory containing TreeMap masks
    split_path = os.path.join("data", "splits", "enmap_treemap", "{}.txt")
    ...
    def __init__(
        ...
        """Initialize the EnMAP TreeMap dataset.
        
        Args:
            root: Root directory where dataset is stored
            sensor: Sensor name (default: "enmap")
            split: Dataset split ("train", "val", or "test")
            classes: List of class codes to include (must contain 0)
        ...
        Raises:
            ValueError: If split is invalid, classes is None, or 0 not in classes
        """
